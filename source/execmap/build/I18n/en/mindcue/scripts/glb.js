...
function listExipryComp() {
	...
	if (document.forms[0].expriyComp[0].checked) {	
		...
		if ( trim(document.forms[0].noDays.value) == "") { 
			alert("Number of days is a required field, Please enter.");
		...
		flag = true;
		if (!( trim(document.forms[0].noDays.value) == "")) { 
			alert("Since you have selected 'List all enabled companies' , 'Number of days to expire' should be empty");
			flag = false;
			document.forms[0].noDays.focus();
			return ;
		}
