...
/**
 * Action class for the NFU assessment activity.
 * It is used by the NFU assessments which checks the multi-task skill
 * of the user.
 */
public class NfuAssessmentAction extends ExecmapAction {
    /**
        ...
        if (pageNo != 0 && pageNoFromUrl == null) {
            ...
            if (questionNo > 0) {
                if (pageNo == ((questionNo *3) -2)) {
                    ...
                    String slider = assessMultiForm.getSliderpos();
                    if (slider != null && !slider.equals("")) {
                        StringTokenizer st = new StringTokenizer(slider);
                        while (st.hasMoreTokens()) {
                            answerMultiModel = new AnswerListMultiModel();
                            answerMultiModel.answerNoStartPos = (byte) Integer
                ...
                if (pageNo == ((questionNo * 3))) {
                    ansListModel =(AnswerListModel)ansModel.getAnswerList().get(questionNo-1);
                    ansListModel.testQuestionId = Byte.valueOf((Integer
                            .valueOf(questionNo)).byteValue());
                    ansListModel.answerNo =(assessMultiForm.getAnswerNo() == null) ? 0
                            : Byte.valueOf(Byte.parseByte(assessMultiForm
                                    .getAnswerNo()));
                    ansModel.getAnswerList().remove(questionNo-1);
                    ansModel.addAnswerModel(ansListModel);
                }
            }
